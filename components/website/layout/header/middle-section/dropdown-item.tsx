...
const DropdownItem = ({ label, summary }: Props) => {
    return (
        <a href={`/${label.toLocaleLowerCase()}`} className='group/item dark:bg-slate-950 flex h-full w-96 flex-col justify-between rounded bg-slate-50 px-3 py-2 transition duration-200 hover:cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 xl:w-56 2xl:w-64'>
            <div>
                <div className='flex items-center space-x-2 pb-[1px]'>
                    {/* <i className={`pt-[1px] ${getActivityIcon(label)}`} /> */}
                    <span className='font-medium'>{label}</span>
                </div>
                <div className='mt-1 text-slate-700 dark:text-slate-400'>{summary}</div>
            </div>
